...
6. Run <code>crontab -l</code> and check if groups:update is in the crontab. If it is, ensure it's <code>RAILS_ENV=development</code>. If the environment is production, run <code>whenever --update-crontab --set environment='development'
